...
pub(crate) fn open_native(
    ...
    let path = get_database_universe_path(name, dir);
...
fn compact_instance(
    ...
    let file_size = fs::metadata(&path)
        .map_err(|_| DatabaseUniverseError::PathError {})?
        .len();
...
pub(crate) fn get_database_universe_path(name: &str, dir: &str) -> String {
    let mut file_name = name.to_string();
    file_name.push_str(".database_universe");
