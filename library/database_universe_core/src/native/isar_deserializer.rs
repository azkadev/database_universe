...
pub(crate) struct DatabaseUniverseDeserializer<'a> {
    pub bytes: &'a [u8],
    static_size: u32,
}

impl<'a> DatabaseUniverseDeserializer<'a> {
    #[inline]
    ...
    #[inline]
    pub fn read_nested(&self, offset: u32) -> Option<DatabaseUniverseDeserializer<'a>> {
        let offset = self.get_offset(offset)?;
    ...
    ) -> Option<(DatabaseUniverseDeserializer<'a>, u32)> {
...
mod test {
    ...
    fn test_read_bool_contains_offset() {
        ...
        let deserializer = DatabaseUniverseDeserializer::from_bytes(&bytes);
