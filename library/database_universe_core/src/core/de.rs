...
use super::error::DatabaseUniverseError;
use super::insert::DatabaseUniverseInsert;
use super::instance::DatabaseUniverseInstance;
...
pub(super) struct DatabaseUniverseJsonImportVisitor<'a, I: DatabaseUniverseInstance, F: Fn(&str) -> i64> {
    instance: &'a I,
...
impl<'a, I: DatabaseUniverseInstance, F: Fn(&str) -> i64> DatabaseUniverseJsonImportVisitor<'a, I, F> {
    pub(super) fn new(instance: &'a I, txn: I::Txn, collection_index: u16, str_to_id: F) -> Self {
        DatabaseUniverseJsonImportVisitor {
            instance,
    ...
    fn import(&self, objects: &[&str]) -> Result<(), DatabaseUniverseError> {
        ...
        for object in objects {
            ...
            let visitor = DatabaseUniverseObjectVisitor::new(insert, &self.str_to_id);
            let (id, writer) =
                deser
                    .deserialize_map(visitor)
                    .map_err(|e| DatabaseUniverseError::JsonError {
                        message: e.to_string(),
                    })?;
            insert = writer;
            ...
            if let Some(id) = id {
                ...
                return Err(DatabaseUniverseError::JsonError {
...
impl<'a, 'de, I: DatabaseUniverseInstance, F: Fn(&str) -> i64> Visitor<'de>
    for DatabaseUniverseJsonImportVisitor<'a, I, F>
{
...
struct DatabaseUniverseObjectVisitor<'a, W: DatabaseUniverseWriter<'a>, F: Fn(&str) -> i64> {
    writer: W,
    str_to_id: &'a F,
}

impl<'a, W: DatabaseUniverseWriter<'a>, F: Fn(&str) -> i64> DatabaseUniverseObjectVisitor<'a, W, F> {
    pub fn new(writer: W, str_to_id: &'a F) -> Self {
        DatabaseUniverseObjectVisitor { writer, str_to_id }
    }
}
...
impl<'a, 'de: 'a, W: DatabaseUniverseWriter<'a>, F: Fn(&str) -> i64> Visitor<'de>
    for DatabaseUniverseObjectVisitor<'a, W, F>
{
                    ...
                    DataType::Object => {
                        ...
                        if let Some(value) = value {
                            if let Some(object_writer) = self.writer.begin_object(index as u32) {
                                ...
                                let visitor = DatabaseUniverseObjectVisitor::new(object_writer, self.str_to_id);
                                let (_, object) = deser
                                    .deserialize_map(visitor)
                                ...
                                self.writer.begin_list(index as u32, list.len() as u32)
                            {
                                for (i, value) in list.iter().enumerate() {
                                    if let Some(value) = value {
                                        ...
                                        let visitor =
                                            DatabaseUniverseObjectVisitor::new(object, self.str_to_id);
                                        let (_, object) = deser
                                            .deserialize_map(visitor)
                                            .map_err(|e| Error::custom(e))?;
