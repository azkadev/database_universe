use super::cursor::{DatabaseUniverseCursor, DatabaseUniverseQueryCursor};
use super::de::DatabaseUniverseJsonImportVisitor;
use super::error::{DatabaseUniverseError, Result};
use super::insert::DatabaseUniverseInsert;
use super::query_builder::DatabaseUniverseQueryBuilder;
use super::reader::DatabaseUniverseReader;
use super::schema::DatabaseUniverseSchema;
...
pub trait DatabaseUniverseInstance: Sized {
    type Instance;
    ...
    type Cursor<'a>: DatabaseUniverseCursor<Reader<'a> = Self::Reader<'a>>
    where
        Self: 'a;

    type Insert<'a>: DatabaseUniverseInsert<'a, Txn = Self::Txn>
    where
        Self: 'a;

    type QueryBuilder<'a>: DatabaseUniverseQueryBuilder<Query = Self::Query>
    where
        Self: 'a;

    type Query;

    type QueryCursor<'a>: DatabaseUniverseQueryCursor<Reader<'a> = Self::Reader<'a>>
    where
        Self: 'a;
    ...
    fn import_json<'a, T: Deserializer<'a>>(
        ...
        let (txn, count) = deserializer
            .deserialize_seq(DatabaseUniverseJsonImportVisitor::new(
                self,
                txn,
                collection_index,
                str_to_id,
