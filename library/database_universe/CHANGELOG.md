...
This version does not support database migration yet and cannot open DatabaseUniverse v3 databases. The stable version will be released in a few weeks and will support migration from v3.
...
- DatabaseUniverse links have been removed in favor of embedded objects
...
- `directory` is now required for `DatabaseUniverse.open()` and `DatabaseUniverse.openSync()`

### Fixes

- Fixed a crash that occasionally occurred when opening DatabaseUniverse
...
- Improved performance for all operations
- Added `maxSizeMiB` option to `DatabaseUniverse.open()` to specify the maximum size of the database file
...
- Fixed crash while opening DatabaseUniverse
...
- Completely new DatabaseUniverse inspector that does not need to be installed anymore
...
- Vastly improved DatabaseUniverse binary format space efficiency resulting in about 20% smaller databases
- Added `id`, `byte`, `short` and `float` typedefs
- `DatabaseUniverseLinks` now support all `Set` methods based on the DatabaseUniverse `Id` of objects
- Added `download` option to `DatabaseUniverse.initializeDatabaseUniverseCore()` to download binaries automatically
- Added `replace` option for indexes
- Added verification for correct DatabaseUniverse binary version
...
- The default DatabaseUniverse name is now `default` and stored in `dir/name.database_universe` and `dir/name.database_universe.lock`
- On non-web platforms, `DatabaseUniverseLink` and `DatabaseUniverseLinks` will load automatically
- `.putSync()`, `.putAllSync()` etc. will now save links recursively by default
- Added `database_universe.getSize()` and `database_universe.getSizeSync()`
...
- Added `compactOnLaunch` conditions to `DatabaseUniverse.open()` for automatic database compaction
- Added `database_universe.copyToFile()` which copies a compacted version of the database to a path
...
- Added `database_universe.verify()` and `col.verify()` methods for checking database integrity in unit tests
...
- Removed `database_universe` parameter from `DatabaseUniverse.txn()`, `DatabaseUniverse.writeTxn()`, `DatabaseUniverse.txnSync()` and `DatabaseUniverse.writeTxnSync()`
...
- DatabaseUniverse will no longer create the provided directory. Make sure it exists before opening an DatabaseUniverse Instance.
- Changed the default index type for all `List`s to `IndexType.hash`
- Renamed `database_universe.getCollection()` to `database_universe.collection()`
...
- Renamed `DatabaseUniverse.initializeLibraries()` to `DatabaseUniverse.initializeDatabaseUniverseCore()`
...
- Significantly reduced DatabaseUniverse Core binary size (about 1.4MB -> 800KB)

### Minor Breaking

- Changed `initializeLibraries(Map<String, String> libraries)` to `initializeLibraries(Map<DatabaseUniverseAbi, String> libraries)`
- Changed min Dart SDK to `2.16.0`

### Fixes

- Fixed issue with `DatabaseUniverseLink.saveSync()`
- Fixed `id` queries
- Fixed error thrown by `BroadcastChannel` in Firefox
- Fixed DatabaseUniverse Inspector connection issue
...
- `database_universe.getCollection()` no longer requires the name of the collection
- `DatabaseUniverse.instanceNames` now returns a `Set` instead of a `List`
...
- Reduced DatabaseUniverse web code size by 50%
- Made `directory` parameter of `DatabaseUniverse.open()` optional for web
- Made `name` parameter of `DatabaseUniverse.getInstance()` optional
- Added `DatabaseUniverse.defaultName` constant
...
DatabaseUniverse now has full web support 🎉. No changes to your code required, just run it.
...
- Added `deleteFromDisk` option to `database_universe.close()`
- Added `.reset()` and `.resetSync()` methods to `DatabaseUniverseLink` and `DatabaseUniverseLinks`
...
- `DatabaseUniverseLink` and `DatabaseUniverseLinks` will no longer be initialized by DatabaseUniverse and must not be `nullable` or `late`.
- Dart `2.14` or higher is required

### Enhancements

- Added API docs for all public methods
- Added `database_universe.clear()`, `database_universe.clearSync()`, `col.clear()` and `col.clearSync()`
...
- Added `DatabaseUniverse.autoIncrement` to allow non-nullable auto-incrementing ids
- `DatabaseUniverse.close()` now returns whether the last instance was closed
...
- Improved DatabaseUniverse Core error messages
...
- `DatabaseUniverseLink` and `DatabaseUniverseLinks` can now be final
...
- Fixed DatabaseUniverseInspector issues (thanks to [RubenBez](https://github.com/RubenBez) and [rizzi37](https://github.com/rizzi37))
