...
class _DatabaseUniverseAnalyzer {
  ObjectInfo analyzeCollection(Element element) {
    ...
    for (final propertyElement in modelClass.allAccessors) {
      ...
      if (!isId || property.type == DatabaseUniverseType.string) {
        index++;
      }
    }
  ...
  PropertyInfo _analyzePropertyInfo(
    ...
    late final DatabaseUniverseType type;
    if (dartType.scalarType.element is EnumElement) {
      ...
      final enumDatabaseUniverseType = enumProperty == null
          ? DatabaseUniverseType.byte
          : enumProperty.type.propertyType;
      if (enumDatabaseUniverseType != DatabaseUniverseType.byte &&
          enumDatabaseUniverseType != DatabaseUniverseType.int &&
          enumDatabaseUniverseType != DatabaseUniverseType.long &&
          enumDatabaseUniverseType != DatabaseUniverseType.string) {
        _err('Unsupported enum property type.', enumProperty);
      }

      type = dartType.isDartCoreList
          ? enumDatabaseUniverseType!.listType
          : enumDatabaseUniverseType!;
      enumMap = {};
    ...
    final elementNullable = type.isList
        ...
        : null;
    if (isId) {
      if (type != DatabaseUniverseType.long &&
    ...
    if ((type == DatabaseUniverseType.byte && nullable) ||
        (type == DatabaseUniverseType.byteList && (elementNullable ?? false))) {
  ...
  String _defaultValue(DartType type) {
    if (type.nullabilitySuffix == NullabilitySuffix.question ||
      ...
      if (type.propertyType == DatabaseUniverseType.byte) {
        return '0';
      } else if (type.propertyType == DatabaseUniverseType.int) {
        return '$_nullInt';
      } else {
        return '$_nullLong';
      }
  ...
  Iterable<IndexInfo> analyzeObjectIndex(
    ...
    for (final index in element.indexAnnotations) {
      ...
      for (var i = 0; i < indexProperties.length; i++) {
        ...
        } else if (property.type == DatabaseUniverseType.json) {
      ...
      _checkDatabaseUniverseName(name, element);
