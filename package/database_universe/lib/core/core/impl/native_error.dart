    ...
    switch (this) {
      ...
      default:
        final length = DatabaseUniverseCore.b
            .database_universe_get_error(DatabaseUniverseCore.stringPtrPtr);
        final ptr = DatabaseUniverseCore.stringPtr;
        if (length != 0 && !ptr.isNull) {
          final error = utf8.decode(ptr.asU8List(length));
          throw DatabaseError(error);
        } else {
          throw DatabaseError(
            'There was an error but it could not be loaded from DatabaseUniverseCore.',
          );
        }
    }
  }
