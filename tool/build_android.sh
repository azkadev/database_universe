...
if [ "$1" = "x64" ]; then
  ...
  mv "target/x86_64-linux-android/release/libDatabaseUniverse.so" "libDatabaseUniverse_android_x64.so"
elif [ "$1" = "armv7" ]; then
  rustup target add armv7-linux-androideabi
  cargo build --target armv7-linux-androideabi --features sqlcipher-vendored --release
  mv "target/armv7-linux-androideabi/release/libDatabaseUniverse.so" "libDatabaseUniverse_android_armv7.so"
else
  rustup target add aarch64-linux-android
  cargo build --target aarch64-linux-android --features sqlcipher-vendored --release
  mv "target/aarch64-linux-android/release/libDatabaseUniverse.so" "libDatabaseUniverse_android_arm64.so"
fi
