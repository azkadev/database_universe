if [ "$1" = "x64" ]; then
  ...
  mv "target/x86_64-unknown-linux-gnu/release/libDatabaseUniverse.so" "libDatabaseUniverse_linux_x64.so"
else
  rustup target add aarch64-unknown-linux-gnu
  cargo build --target aarch64-unknown-linux-gnu --features sqlcipher-vendored --release
  mv "target/aarch64-unknown-linux-gnu/release/libDatabaseUniverse.so" "libDatabaseUniverse_linux_arm64.so"
fi
