...
use super::{data_type::DataType, ser::DatabaseUniverseObjectSerialize};
use serde::Serializer;
use std::borrow::Cow;

pub trait DatabaseUniverseReader {
    type ObjectReader<'b>: DatabaseUniverseReader
    where
        Self: 'b;

    type ListReader<'b>: DatabaseUniverseReader
    where
        Self: 'b;
    ...
    where
        Self: Sized,
    {
        let obj = DatabaseUniverseObjectSerialize::new(self);
        serializer
            .serialize_some(&obj)
