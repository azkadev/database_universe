use super::{data_type::DataType, reader::DatabaseUniverseReader};
use serde::ser::{SerializeMap, SerializeSeq};
use serde::Serialize;
use serde_json::Value;

pub(super) struct DatabaseUniverseObjectSerialize<'a, R: DatabaseUniverseReader> {
    reader: &'a R,
}

impl<'a, R: DatabaseUniverseReader> DatabaseUniverseObjectSerialize<'a, R> {
    pub fn new(reader: &'a R) -> Self {
        DatabaseUniverseObjectSerialize { reader }
    }
}

impl<'a, R: DatabaseUniverseReader> Serialize for DatabaseUniverseObjectSerialize<'a, R> {
            ...
            match data_type {
                ...
                DataType::Object => {
                    if let Some(object) = self.reader.read_object(index as u32) {
                        let reader = DatabaseUniverseObjectSerialize::new(&object);
                ...
                _ => {
                    ...
                    match (element_type, list) {
                        (Some(element_type), Some((list, length))) => {
                            let reader = DatabaseUniverseListSerialize::new(element_type, &list, length);
                            ser.serialize_entry(name, &reader)?;
                        }
                        _ => {}
                    }
...
struct DatabaseUniverseListSerialize<'a, R: DatabaseUniverseReader> {
    element_type: DataType,
    reader: &'a R,
    length: u32,
}

impl<'a, R: DatabaseUniverseReader> DatabaseUniverseListSerialize<'a, R> {
    fn new(element_type: DataType, reader: &'a R, length: u32) -> Self {
        DatabaseUniverseListSerialize {
            element_type,
            reader,
            length,
        }
...
impl<'a, R: DatabaseUniverseReader> Serialize for DatabaseUniverseListSerialize<'a, R> {
    fn serialize<S>(&self, serializer: S) -> Result<S::Ok, S::Error>
    where
        S: serde::Serializer,
    {
