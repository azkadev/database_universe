...
use super::{data_type::DataType, error::DatabaseUniverseError};
use itertools::Itertools;
use serde::{Deserialize, Serialize};

#[derive(Serialize, Deserialize, Clone, Hash, Debug, PartialEq, Eq)]
pub struct DatabaseUniverseSchema {
    pub name: String,
...
impl DatabaseUniverseSchema {
    ...
    pub fn from_json(json: &[u8]) -> Result<Vec<Self>> {
        if let Ok(collections) = serde_json::from_slice::<Vec<DatabaseUniverseSchema>>(json) {
            Ok(collections)
        } else {
    ...
    pub fn find_changes(
        &self,
        old_collection: &DatabaseUniverseSchema,
    ) -> (
    ...
    fn verify(&self, collections: &[DatabaseUniverseSchema]) -> Result<()> {
...
fn schema_error<T>(msg: &str) -> Result<T> {
    Err(DatabaseUniverseError::SchemaError {
...
mod test {
    ...
    fn test_verify_checks_name() {
        let schema = DatabaseUniverseSchema::new("test", None, Vec::new(), Vec::new(), false);
        assert!(schema.verify(&[]).is_ok());

        let schema = DatabaseUniverseSchema::new("_test", None, Vec::new(), Vec::new(), false);
        assert!(schema.verify(&[]).is_err());

        let schema = DatabaseUniverseSchema::new("sqlite_test", None, Vec::new(), Vec::new(), false);
        assert!(schema.verify(&[]).is_err());

        let schema = DatabaseUniverseSchema::new("", None, Vec::new(), Vec::new(), false);
        assert!(schema.verify(&[]).is_err());
    }

    #[test]
    fn test_verify_checks_index_names() {
        let schema = DatabaseUniverseSchema::new(
            "test",
            None,
            Vec::new(),
    ...
    fn test_verify_checks_non_object_property_has_no_target() {
        let schema1 = DatabaseUniverseSchema::new(
            "test",
            None,
        ...
        let schema2 = DatabaseUniverseSchema::new("test2", None, vec![], Vec::new(), false);
    ...
    fn test_verify_checks_index_properties_are_not_object() {
        ...
        let schema2 = DatabaseUniverseSchema::new("test2", None, vec![], vec![], true);
