...
impl NativeIndex {
    ...
    fn create_key(&self, object: DatabaseUniverseDeserializer, buffer: Vec<u8>) -> (Vec<u8>, bool) {
    ...
    pub fn create_for_object<F>(
        ...
        txn: &NativeTxn,
        id: i64,
        object: DatabaseUniverseDeserializer,
        buffer: Vec<u8>,
        mut delete: F,
    ) -> Result<Vec<u8>>
    where
    ...
    /* pub fn iter_between<'txn, 'env>(
        &self,
        cursors: &DatabaseUniverseCursors<'txn, 'env>,
        lower_key: &IndexKey,
        upper_key: &IndexKey,
    ...
    pub fn get_size(&self, cursors: &DatabaseUniverseCursors) -> Result<u64> {
        Ok(cursors.db_stat(self.db)?.1)
    }

    pub fn clear(&self, cursors: &DatabaseUniverseCursors) -> Result<()> {
        cursors.clear_db(self.db)
    }

    pub fn verify(&self, cursors: &DatabaseUniverseCursors, objects: &IntMap<DatabaseUniverseObject>) -> Result<()> {
        let mut count = 0;
        ...
        for id in objects.keys() {
            ...
            key_builder.create_keys(object, |key| {
                ...
                if result.is_some() {
                    ...
                    Err(DatabaseUniverseError::DbCorrupted {
