...
pub(crate) fn mdbx_result(err_code: c_int) -> Result<()> {
    match err_code {
        ...
        mdbx_sys::MDBX_MAP_FULL => Err(DatabaseUniverseError::DbFull {}),
        other => Err(mdbx_error(other)),
    }
}

#[inline]
pub(crate) fn mdbx_error(err_code: c_int) -> DatabaseUniverseError {
    match err_code {
        mdbx_sys::MDBX_MAP_FULL => DatabaseUniverseError::DbFull {},
        other => unsafe {
...
pub(crate) mod osal {
    ...
    pub fn str_to_os(str: &str) -> Result<U16CString> {
        U16CString::from_str(str).map_err(|_| DatabaseUniverseError::IllegalString {})
    }
...
pub(crate) mod osal {
    ...
    pub fn str_to_os(str: &str) -> Result<CString> {
        CString::new(str.as_bytes()).map_err(|_| DatabaseUniverseError::IllegalString {})
    }
