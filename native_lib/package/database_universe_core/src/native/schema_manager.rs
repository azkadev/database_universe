...
use crate::core::schema::{DatabaseUniverseSchema, PropertySchema};
use itertools::Itertools;
use std::borrow::Cow;
use std::sync::Arc;

const DATABASE_UNIVERSE_FILE_VERSION: u8 = 3;

pub(crate) fn perform_migration(
    instance_id: u32,
    env: &Arc<Env>,
    mut schemas: Vec<DatabaseUniverseSchema>,
    ...
    for schema in schemas.iter_mut() {
        ...
        schema.version = DATABASE_UNIVERSE_FILE_VERSION;
...
fn get_schemas(txn: &NativeTxn, info_db: Db) -> Result<Vec<DatabaseUniverseSchema>> {
    let info_cursor = txn.get_cursor(info_db)?;
    let mut schemas = vec![];
    for (_, bytes) in info_cursor.iter()? {
        let col =
            serde_json::from_slice::<DatabaseUniverseSchema>(bytes).map_err(|_| DatabaseUniverseError::SchemaError {
...
fn save_schema(txn: &NativeTxn, info_db: Db, schema: &DatabaseUniverseSchema) -> Result<()> {
    let mut info_cursor = txn.get_cursor(info_db)?;
    let bytes = serde_json::to_vec(schema).map_err(|_| DatabaseUniverseError::SchemaError {
...
fn delete_collection(txn: &NativeTxn, info_db: Db, schema: &DatabaseUniverseSchema) -> Result<()> {
...
fn migrate_collection(
    ...
    existing_schema: &DatabaseUniverseSchema,
) -> Result<Vec<PropertySchema>> {
    if existing_schema.version != DATABASE_UNIVERSE_FILE_VERSION {
        return Err(DatabaseUniverseError::VersionError {});
    }
