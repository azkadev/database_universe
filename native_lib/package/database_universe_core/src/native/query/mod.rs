...
impl NativeQuery {
    ...
    pub(crate) fn aggregate(
        ...
        match aggregation {
            ...
            Aggregation::Count => Some(DatabaseUniverseValue::Integer(iterator.count() as i64)),
            Aggregation::IsEmpty => Some(DatabaseUniverseValue::Bool(iterator.next().is_none())),
        }
    }
...
impl QueryMatches for NativeQuery {
    type Object<'a> = DatabaseUniverseDeserializer<'a>;

    fn matches<'a>(&self, id: i64, object: &DatabaseUniverseDeserializer<'a>) -> bool {
...
impl<'a> DatabaseUniverseQueryCursor for NativeQueryCursor<'a> {
