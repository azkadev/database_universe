...
pub(crate) struct SortedQueryIterator<'txn> {
    iterator: Take<Skip<IntoIter<(i64, DatabaseUniverseDeserializer<'txn>)>>>,
}
...
impl<'a> SortedQueryIterator<'a> {
    ...
    fn compare_property(
        o1: &DatabaseUniverseDeserializer,
        o2: &DatabaseUniverseDeserializer,
        offset: u32,
        data_type: DataType,
        case_sensitive: bool,
    ) -> Ordering {
...
impl<'txn> Iterator for SortedQueryIterator<'txn> {
    type Item = (i64, DatabaseUniverseDeserializer<'txn>);
