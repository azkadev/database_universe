...
impl NativeCollection {
    ...
    pub fn get_cursor<'a>(&self, txn: &'a NativeTxn) -> Result<TxnCursor<'a>> {
        let db = self.db.ok_or(DatabaseUniverseError::UnsupportedOperation {})?;
        txn.get_cursor(db)
    }
    ...
    pub fn update<'a>(
        ...
        if let Some((_, old_object)) = cursor.move_to(&id.to_id_bytes())? {
            ...
            let mut new_object = DatabaseUniverseSerializer::new(buffer, 0, self.static_size);
    ...
    fn write_value(
        &self,
        object: &mut DatabaseUniverseSerializer,
        property_index: u16,
        value: Option<&DatabaseUniverseValue>,
    ) -> Result<()> {
        if let Some(p) = self.get_property(property_index) {
            match (value, p.data_type) {
                (None, _) => object.write_null(p.offset, p.data_type),
                (Some(DatabaseUniverseValue::Bool(value)), DataType::Bool) => {
                    object.write_bool(p.offset, *value)
                }
                (Some(DatabaseUniverseValue::Integer(value)), DataType::Byte) => {
                    object.write_byte(p.offset, *value as u8)
                }
                (Some(DatabaseUniverseValue::Integer(value)), DataType::Int) => {
                    object.write_int(p.offset, *value as i32)
                }
                (Some(DatabaseUniverseValue::Integer(value)), DataType::Long) => {
                    object.write_long(p.offset, *value)
                }
                (Some(DatabaseUniverseValue::Real(value)), DataType::Float) => {
                    object.write_float(p.offset, *value as f32)
                }
                (Some(DatabaseUniverseValue::Real(value)), DataType::Double) => {
                    object.write_double(p.offset, *value)
                }
                (Some(DatabaseUniverseValue::String(value)), DataType::String)
                | (Some(DatabaseUniverseValue::String(value)), DataType::Json) => {
