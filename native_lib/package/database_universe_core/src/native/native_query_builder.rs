...
impl<'a> DatabaseUniverseQueryBuilder for NativeQueryBuilder<'a> {
...
fn condition_to_native(
    ...
    let filter = match condition.condition_type {
        ...
        ConditionType::StringStartsWith => {
            ...
            let upper = format!("{}{}", lower, DatabaseUniverseValue::MAX_STRING);
            NativeFilter::string(
                property?,
                Some(lower),
                Some(&upper),
...
fn lower_real(value: Option<&DatabaseUniverseValue>, include: bool) -> Option<f64> {
...
fn upper_real(value: Option<&DatabaseUniverseValue>, include: bool) -> Option<f64> {
    let mut value = if let Some(value) = value {
        value.real()?
    } else {
        f64::NAN
...
fn get_max(property: Option<&NativeProperty>) -> Option<DatabaseUniverseValue> {
    let value = if let Some(property) = property {
        match property.data_type {
            DataType::Bool | DataType::BoolList => DatabaseUniverseValue::Bool(true),
            DataType::Byte | DataType::ByteList => DatabaseUniverseValue::Integer(u8::MAX as i64),
            DataType::Int | DataType::IntList => DatabaseUniverseValue::Integer(i32::MAX as i64),
            DataType::Float | DataType::FloatList => DatabaseUniverseValue::Real(f64::INFINITY),
            DataType::Long | DataType::LongList => DatabaseUniverseValue::Integer(i64::MAX),
            DataType::Double | DataType::DoubleList => DatabaseUniverseValue::Real(f64::INFINITY),
            DataType::String | DataType::StringList | DataType::Json => {
                DatabaseUniverseValue::String(DatabaseUniverseValue::MAX_STRING.to_string())
            }
            DataType::Object | DataType::ObjectList => return None,
        }
    } else {
        DatabaseUniverseValue::Integer(i64::MAX)
    };
    Some(value)
}
