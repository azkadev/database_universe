...
extension ChatbotExtensionExampleClientFlutter on ExampleClientDatabase {
  ...
  String getRespondFromPromptText({
    ...
    final dataOld = databaseUniverseCore.chatbotDataLocalDatabases.where().anyOf([
      prompt,
    ], (e, value) {
  ...
  bool savePromptAndRespond({
    ...
    final chatbotDataLocalDatabaseOld = databaseUniverseCore.chatbotDataLocalDatabases.where().promptMatches(prompt, caseSensitive: false).findFirst();
    if (chatbotDataLocalDatabaseOld == null) {
      final ChatbotDataLocalDatabase chatbotDataLocalDatabaseNew = ChatbotDataLocalDatabase();
      chatbotDataLocalDatabaseNew.id = databaseUniverseCore.chatbotDataLocalDatabases.autoIncrement();
      ...
      databaseUniverseCore.write((databaseUniverseCore) {
        databaseUniverseCore.chatbotDataLocalDatabases.put(chatbotDataLocalDatabaseNew);
        return;
      });
      return true;
    }
    ...
    databaseUniverseCore.write((databaseUniverseCore) {
      databaseUniverseCore.chatbotDataLocalDatabases.put(chatbotDataLocalDatabaseOld);
      return;
    });
    return true;
  }
